...
namespace ft {
	...
	public:
		...
		void print(node_ptr p,int start)
		{
			...
			std::cout << ((p->null_node == false) ? p->data.first : 0)  << " " << "(" << p->count << ") " << ((p->color == 1) ? "B" : "R") << std::endl;
			if (p->left != NULL)
			{
				print(p->left, start);
			}
		...
		node_ptr insert_data(value_type data) {
			node_ptr node = new multimap_tree_node<Key, Mapped, T>(data, RED);
			if (this->root == NULL) {
				this->root = node;
				node->color = BLACK;
			}
			...
			node->right = new multimap_tree_node<Key, Mapped, T>(data, BLACK, true, 0);
			node->left = new multimap_tree_node<Key, Mapped, T>(data, BLACK, true, 0);
